...
const [modalidadSeleccionada, setModalidadSeleccionada] = React.useState("F11");
const [sistemaSeleccionado, setSistemaSeleccionado] = React.useState("4-3-3");
const jugadores = [
  { id: 1, nombre: "Juan", dorsal: 9 },
  { id: 2, nombre: "Pedro", dorsal: 10 },
  { id: 3, nombre: "Luis", dorsal: 7 },
  { id: 4, nombre: "Carlos", dorsal: 5 },
  { id: 5, nombre: "Andrés", dorsal: 11 },
  { id: 6, nombre: "Miguel", dorsal: 4 },
  { id: 7, nombre: "Pablo", dorsal: 6 },
  { id: 8, nombre: "Raúl", dorsal: 8 },
  { id: 9, nombre: "Javi", dorsal: 2 },
  { id: 10, nombre: "Hugo", dorsal: 3 },
  { id: 11, nombre: "Rubén", dorsal: 1 },
];
  // Ajustes de partido
...
const invertSystemSide = () => {
              ...
              <Card title={`📋 ${t.modality}`} subtitle={`${t.playersOnFieldLimit}: ${maxOnField}`}>
                <div className="flex flex-wrap gap-2 items-center">
                  {Object.keys(modalityToMax).map(m => (
                    <button key={m} className={`px-3 py-2 border rounded ${modality===m?'bg-blue-600 text-white':'bg-white'}`} onClick={()=>{
  setModality(m);
  const primerSistema = {
    'f11': '4-3-3',
    'f9': '4-3-1',
  ...
  {(() => {
    const sistemas = {
      'f11': ['4-3-3','4-4-2','4-2-3-1','3-5-2','3-4-3','5-3-2','Personalizado'],
      'f9': ['4-3-1','3-4-1','3-3-2','Personalizado'],
      'f8': ['4-2-1','3-3-1','3-2-2','3-1-3','2-4-1','Personalizado'],
      'f7': ['3-2-1','2-3-1','2-2-2','3-1-2','Personalizado'],
      'f5': ['2-1-1','1-2-1','1-1-2','3-1-0','Personalizado'],
    };
    return (sistemas[modality] || sistemas['f11']).map(s =>
...
<Campograma
  modalidad={modality}
  sistema={systemName}
  jugadores={systemSlots.slice(0, maxOnField).map((slot, idx) => {
    const p = roster.find(r => r.id === slot.playerId);
    return {
      id: slot.id || idx,
      dorsal: p ? p.number : idx + 1,
      nombre: p ? p.name : `Slot ${idx + 1}`,
    };
  })}
  onLayoutChange={(nuevoLayout) => {
    // aquí puedes guardar las posiciones si quieres
  }}
/>
                  </div>
