...
export default function App() {
  ...
  const [ownTeamName, setOwnTeamName] = useState(() => localStorage.getItem('ffd-own-team-name') || '');
  ...
  // Sincronizar nombres de equipos según configuración
  useEffect(() => {
    if (ownTeamName && opponentName) {
      if (ownTeamSide === 'home') {
        setHomeName(ownTeamName);
        setAwayName(opponentName);
      } else {
        setHomeName(opponentName);
        setAwayName(ownTeamName);
      }
    } else {
  ...
  useEffect(() => { localStorage.setItem('ffd-own-team-name', ownTeamName); }, [ownTeamName]);
                  ...
                  <>
                    <Card title="⚙️ Competición y partido" subtitle="Configura datos del encuentro">
                      <div className="grid sm:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-xs text-slate-500 mb-1">Tu equipo juega como</label>
                          <select 
                            className="border rounded px-2 py-2 w-full" 
                            value={ownTeamSide} 
                            onChange={(e)=>setOwnTeamSide(e.target.value)}
                          >
              ...
              <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
                ...
                {!isPro ? (
                  ...
                  <>
                    ...
                    {/* Últimos 5 partidos */}
                    <Card title={t.last5Results} subtitle="G=Ganado, E=Empatado, P=Perdido">
