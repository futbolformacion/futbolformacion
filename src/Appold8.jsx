...
<Campograma
  ...
  jugadores={jugadores}
  onLayoutChange={(nuevoLayout) => setLayout(nuevoLayout)}
/>
...
// Reemplaza tu buildSystemPreset por esta versión
function buildSystemPreset(systemName, maxOnField, attackRight = true) {
  // Helper para crear filas uniformes a lo ancho del campo
  const row = (y, n, xStart = 10, xEnd = 90) => {
    if (n <= 0) return [];
    const gap = (xEnd - xStart) / (n + 1);
    return Array.from({ length: n }, (_, i) => ({
      id: `slot-${y}-${i}`,
      x: Math.round(xStart + gap * (i + 1)),
      y,
      playerId: ''
    }));
  };

  // Ajustes de “altura” por línea según modalidad (más compacto en F5)
  const grids = {
    F5: { gk: 50, def: 72, mid: 50, att: 28 },
    F7: { gk: 50, def: 70, mid: 50, att: 30 },
    F8: { gk: 50, def: 72, mid: 52, att: 32 },
    F9: { gk: 50, def: 72, mid: 52, att: 32 },
    F11: { gk: 50, def: 78, mid: 52, att: 26 },
  };

  // Detecta modalidad desde maxOnField
  const modality =
    maxOnField === 5 ? 'F5' :
    maxOnField === 7 ? 'F7' :
    maxOnField === 8 ? 'F8' :
  ...
  const G = grids[modality];

  // Normaliza nombre (acepta “Personalizado” → reparte por líneas equilibradas)
  const name = (systemName || '').toLowerCase();

  // GK siempre 1 en línea de portero
  const gk = [{ id: 'slot-gk-0', x: 8, y: G.gk, playerId: '' }];

  // Presets por modalidad
  // Cada preset devuelve [defenders, midfielders, attackers] como cantidades por línea
  const presets = {
    // F5: 1-2-1-1, 1-1-2-1, 1-1-1-1-1 etc. Se simplifican a 3 líneas + GK
    F5: {
      '2-1-1': [2, 1, 1],
      '1-2-1': [1, 2, 1],
      '1-1-2': [1, 1, 2],
      default: [2, 1, 1], // típico 1-2-1 (con GK)
    },
    // F7: comunes 3-2-1, 2-3-1, 2-2-2
    F7: {
      '3-2-1': [3, 2, 1],
      '2-3-1': [2, 3, 1],
      '2-2-2': [2, 2, 2],
    ...
    // F11: estándar clásicos
    F11: {
      '4-3-3': [4, 3, 3],
      '4-4-2': [4, 4, 2],
      '4-2-3-1': [4, 2, 3], // el “1” es punta
      '3-5-2': [3, 5, 2],
      '3-4-3': [3, 4, 3],
      '5-3-2': [5, 3, 2],
      default: [4, 3, 3],
  ...
  // Convierte “Personalizado” a reparto equilibrado según maxOnField
  const toBalanced = (total) => {
    // restando GK
    const outfield = total - 1;
    // reparte aprox 40% def, 35% mid, 25% att
    const d = Math.max(1, Math.round(outfield * 0.4));
    const a = Math.max(1, Math.round(outfield * 0.25));
    const m = Math.max(1, outfield - d - a);
    return [d, m, a];
  };

  const catalog = presets[modality] || presets.F11;
  const key = name.replace(/\s+/g, '');
  let template = catalog[key] || catalog[systemName] || catalog.default;

  if (!template) {
    template = toBalanced(maxOnField);
  }

  // Construye filas: defensas en y=G.def, medios en y=G.mid, delanteros en y=G.att
  const def = row(G.def, template[0]);
  const mid = row(G.mid, template[1]);
  const att = row(G.att, template[2]);

  // Si maxOnField no coincide con suma (por sistemas con 4 líneas), rellena/recorta
  const totalNeeded = maxOnField - 1; // fuera de GK
  let all = [...def, ...mid, ...att];

  if (all.length > totalNeeded) {
    all = all.slice(0, totalNeeded);
  } else if (all.length < totalNeeded) {
    // Añadir atacantes extra centrados si faltan
    const missing = totalNeeded - all.length;
    all = [
      ...all,
      ...row(Math.round((G.mid + G.att) / 2), missing, 40, 60),
    ];
  }

  // Ajuste lado de ataque
  const flipX = (x) => (attackRight ? x : 100 - x);

  const final = [
    ...gk.map(s => ({ ...s, x: flipX(s.x) })),
    ...all.map((s, i) => ({ ...s, x: flipX(s.x), id: s.id || `slot-${i}` })),
  ];
                ...
                <div className="flex flex-col lg:flex-row gap-4">
                  <div className="space-y-2 lg:w-2/3">
                    <div className="flex flex-wrap items-center gap-2">
                      <label className="text-sm">{t.modality}:</label>
                      <span className="px-2 py-1 border rounded bg-slate-50">{modality.toUpperCase()}</span>

                      <label className="text-sm ml-2">Sistema:</label>
                      <select className="border rounded px-2 py-1"
                        value={systemName}
                        onChange={(e)=>applySystem(e.target.value)}>
                        {['4-3-3','4-4-2','4-2-3-1','3-5-2','3-4-3','5-3-2','Personalizado'].map(s=>
                          <option key={s} value={s}>{s}</option>
                        )}
                      </select>

                      <button className="px-2 py-1 border rounded" onClick={saveSystemPreset}>Guardar plantilla</button>
                      <button className="px-2 py-1 border rounded" onClick={()=>loadSystemPreset(systemName)}>Cargar plantilla</button>
                      <button className="px-2 py-1 border rounded" onClick={invertSystemSide}>
                        {attackRight ? 'Invertir (→ a ←)' : 'Invertir (← a →)'}
                      </button>
                    </div>

                    {/* Campo */}
                    <div className="relative w-full" style={{paddingTop:'64.76%'}}>
                      <div style={{position:'absolute', inset:0}}>
                        <svg viewBox="0 0 100 100" width="100%" height="100%" style={{background:'#198754', border:'1px solid #e5e7eb', borderRadius:8}}>
                          ...
                          <rect x="0" y="30" width="16.5" height="40" fill="none" stroke="#e5e7eb" strokeWidth="0.5"/>
                          <rect x="0" y="40" width="5.5" height="20" fill="none" stroke="#e5e7eb" strokeWidth="0.5"/>
                          <rect x="83.5" y="30" width="16.5" height="40" fill="none" stroke="#e5e7eb" strokeWidth="0.5"/>
                          <rect x="94.5" y="40" width="5.5" height="20" fill="none" stroke="#e5e7eb" strokeWidth="0.5"/>
                          <circle cx="50" cy="50" r="9.15" fill="none" stroke="#e5e7eb" strokeWidth="0.5"/>

{(systemSlots && systemSlots.length ? systemSlots : buildSystemPreset(systemName, maxOnField, attackRight)).map((slot, idx) => {
  // 1) Rotación 90° para todos
  let rawX = slot.y;          // horizontal tras rotar
  let rawY = 100 - slot.x;    // vertical tras rotar

  // 2) Detectar portero: primer slot o role==='GK'
  const isGK = slot.role === 'GK' || idx === 0;

  if (isGK) {
    // Queremos al portero DETRÁS de la defensa propia:
    // - Si atacamos a la derecha, nuestra portería está a la izquierda -> xGK cerca de 6
    // - Si atacamos a la izquierda, nuestra portería está a la derecha -> xGK cerca de 94
    const xGK = attackRight ? 6 : 94;
    rawX = xGK;    // pegado a su portería
    rawY = 50;     // centrado vertical
  }

  // 3) Flip horizontal final (mantiene consistencia si inviertes el lado)
  const x = attackRight ? rawX : (100 - rawX);
  const y = rawY;

  const p = roster.find(r => r.id === slot.playerId);
  const fill = p ? '#0ea5e9' : '#e5e7eb';
  const text = p ? `${p.number}` : `${idx + 1}`;

  return (
    <g key={slot.id || idx}>
      <circle cx={x} cy={y} r="3.2" fill={fill} stroke="#111827" strokeWidth="0.3" />
      <text x={x} y={y + 0.5} textAnchor="middle" fontSize="2.6" fill={p ? '#ffffff' : '#111827'} fontWeight="700">
        {text}
      </text>
    </g>
  );
                  ...
                  <div className="lg:w-1/3">
                    <div className="text-sm font-semibold mb-2">Asignar jugadores a slots</div>
                    <div className="space-y-2 max-h-[420px] overflow-auto">
                      {roster.map(p=>(
                        <div key={p.id} className="p-2 border rounded flex items-center justify-between bg-white">
                          <div className="text-sm">
                            <div className="font-semibold">{p.number} - {p.name}</div>
                            <div className="text-xs text-slate-500">{p.position}</div>
                          </div>
                          <div className="flex gap-2">
                            <button className="px-2 py-1 border rounded text-xs"
                              onClick={()=>{
                                const idx = systemSlots.findIndex(s => !s.playerId);
                                if (idx === -1) { alert('No hay slots vacíos'); return; }
                                setSystemSlots(list => list.map((s,i)=> i===idx ? { ...s, playerId: p.id } : s));
                              }}>
                              Asignar
                            </button>
                            <button className="px-2 py-1 border rounded text-xs"
                              onClick={()=>{
                                setSystemSlots(list => list.map(s => s.playerId === p.id ? { ...s, playerId: '' } : s));
                              }}>
                              Quitar
                            </button>
                          </div>
                    ...
                    <div className="mt-3 space-y-2">
                      <button className="w-full px-3 py-2 bg-emerald-600 text-white rounded font-semibold"
                        onClick={()=>{
                          const assignedIds = systemSlots.map(s => s.playerId).filter(Boolean);
                          let count = 0;
                          setRoster(list => list.map(p => {
                            const shouldBeOn = assignedIds.includes(p.id) && count < maxOnField;
                            if (shouldBeOn) count++;
                            return { ...p, onField: shouldBeOn };
                          }));
                          alert(lang==='es' ? 'Sistema aplicado a la alineación.' : 'System applied to lineup.');
                        }}>
                        Aplicar a alineación
                      </button>
                      <button className="w-full px-3 py-2 border rounded"
                        onClick={()=> setSystemSlots(list => list.map(s => ({ ...s, playerId: '' })))}>
