                    ...
                    <div className="relative w-full" style={{paddingTop:'64.76%'}}>
                      <div style={{position:'absolute', inset:0}}>
                        <svg viewBox="0 0 100 100" width="100%" height="100%" style={{background:'#198754', border:'1px solid #e5e7eb', borderRadius:8}}>
                          ...
                          {(systemSlots && systemSlots.length ? systemSlots : buildSystemPreset(systemName, maxOnField)).map((slot, idx) => {
                            const sx = attackRight ? slot.x : (100 - slot.x);
                            const sy = slot.y;
                            ...
                            return (
                              <g key={slot.id || idx}>
                                <circle cx={sx} cy={sy} r="3.2" fill={fill} stroke="#111827" strokeWidth="0.3" />
                                <text x={sx} y={sy+0.5} textAnchor="middle" fontSize="2.6" fill={p ? '#ffffff' : '#111827'} fontWeight="700">{text}</text>
                              </g>
                            );
                          })}
                        </svg>
